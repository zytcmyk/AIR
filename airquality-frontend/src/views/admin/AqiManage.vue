  ...
  <div class="manage-page">
    ...
    <div class="table-card">
      <el-table :data="dataList" v-loading="loading" @selection-change="handleSelectionChange" class="apple-table">
        ...
        <el-table-column prop="aqiValue" label="AQI值" width="100" sortable>
          <template #default="{ row }">
            <span class="aqi-tag" :class="getAqiClass(row.aqiValue)">{{ row.aqiValue }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="qualityLevel" label="质量等级" width="100">
          <template #default="{ row }">
            <span class="quality-tag" :class="getQualityClass(row.qualityLevel)">{{ row.qualityLevel }}</span>
          </template>
        </el-table-column>
    ...
    <!-- 弹窗 -->
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑AQI数据' : '新增AQI数据'" width="480px" class="apple-dialog">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="城市" prop="city">
          <el-input v-model="form.city" placeholder="请输入城市" />
        </el-form-item>
        <el-form-item label="AQI值" prop="aqiValue">
          <el-input-number v-model="form.aqiValue" :min="0" :max="500" style="width: 100%" />
        </el-form-item>
...
import { getAqiList, createAqi, updateAqi, deleteAqi, batchDeleteAqi } from '@/api/admin'
...
const searchForm = reactive({ city: '' })
const pagination = reactive({ page: 1, size: 10, total: 0 })
const form = reactive({ id: null, city: '', aqiValue: 0, date: '' })

const rules = {
  city: [{ required: true, message: '请输入城市', trigger: 'blur' }],
  aqiValue: [{ required: true, message: '请输入AQI值', trigger: 'blur' }],
  date: [{ required: true, message: '请选择日期', trigger: 'change' }]
}

const getAqiClass = (value) => {
  if (value <= 50) return 'level-1'
  if (value <= 100) return 'level-2'
  if (value <= 150) return 'level-3'
  if (value <= 200) return 'level-4'
  return 'level-5'
}

const getQualityClass = (level) => {
  const map = { '优': 'level-1', '良': 'level-2', '轻度污染': 'level-3', '中度污染': 'level-4', '重度污染': 'level-5', '严重污染': 'level-5' }
  return map[level] || ''
}
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await getAqiList(params)
    if (res.code === 200) {
...
const handleReset = () => { searchForm.city = ''; dateRange.value = []; handleSearch() }
const handleSelectionChange = (selection) => { selectedIds.value = selection.map(item => item.id) }

const handleAdd = () => {
  isEdit.value = false
  Object.assign(form, { id: null, city: '', aqiValue: 0, date: '' })
  dialogVisible.value = true
}

const handleEdit = (row) => {
  isEdit.value = true
  Object.assign(form, { id: row.id, city: row.city, aqiValue: row.aqiValue, date: row.date })
  dialogVisible.value = true
}
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) await updateAqi(form.id, form)
    else await createAqi(form)
...
const handleBatchDelete = async () => {
  try {
    ...
    await batchDeleteAqi(selectedIds.value)
    ElMessage.success('删除成功')
    loadData()
  } catch (e) {
    if (e !== 'cancel') ElMessage.error('删除失败')
...
.aqi-tag.level-1 { background: rgba(125, 190, 140, 0.15); color: #7DBE8C; }
.aqi-tag.level-2 { background: rgba(212, 167, 106, 0.15); color: #D4A76A; }
.aqi-tag.level-3 { background: rgba(212, 167, 106, 0.2); color: #C4965A; }
.aqi-tag.level-4 { background: rgba(212, 139, 139, 0.15); color: #D48B8B; }
.aqi-tag.level-5 { background: rgba(176, 139, 212, 0.15); color: #B08BD4; }
...
.quality-tag.level-1 { background: rgba(125, 190, 140, 0.12); color: #7DBE8C; }
.quality-tag.level-2 { background: rgba(212, 167, 106, 0.12); color: #D4A76A; }
.quality-tag.level-3 { background: rgba(212, 167, 106, 0.15); color: #C4965A; }
.quality-tag.level-4 { background: rgba(212, 139, 139, 0.12); color: #D48B8B; }
.quality-tag.level-5 { background: rgba(176, 139, 212, 0.12); color: #B08BD4; }
...
@media (max-width: 900px) {
  ...
  .search-input, .date-picker { width: 100%; }
}
</style>
